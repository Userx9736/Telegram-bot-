# bot.py – Full working version with health server + Instagram proxy fallback
...
# ═══ NEW: HTTP server for Render health checks ═══
...
# ═══════════ INSTAGRAM PROXY LIST (tried in order) ═══════════
# These public mirrors serve Instagram content without a login.
# We try direct URL first, then these backups.
IG_PROXIES = [
    "instagram.com",          # 0 = original
    "ddinstagram.com",        # 1
    "imginn.com",             # 2
    "dumpoir.com",            # 3
    "pictaram.com",           # 4
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not any(d in original_url.lower() for d in SUPPORTED_DOMAINS):
        await update.message.reply_text(
            "❌ *Unsupported Link*\n\n"
    ...
    # ---- Build list of URLs to try (Instagram only) ----
    urls_to_try = [original_url]   # start with the original
    if platform == "instagram":
        # If the URL contains "instagram.com", create proxy versions
        for proxy in IG_PROXIES[1:]:   # skip the first (original) which is already there
            urls_to_try.append(original_url.replace("instagram.com", proxy))
    ...
    for target_url in urls_to_try:
        try:
            info = await get_media_info(target_url)
    ...
    if info is None:
        await update.message.reply_text(
            ...
            f"{friendly_error(last_error)}\n\n"
            f"_{BOT_TAG}_",
            parse_mode=ParseMode.MARKDOWN,
        )
        return

    pending_urls[uid] = working_url
