...
CHANNEL   = "@joinpremiummodsx"   # your channel username with @

# ─────────────────────────────────────────────
#  INSTALOADER  (Instagram direct-link method)
# ─────────────────────────────────────────────
...
# ═════════════════════════════════════════════
#  INSTAGRAM  (direct button — no cookies needed)
# ═════════════════════════════════════════════

async def handle_instagram(url: str, status_msg, uid: int):
    try:
        if "/reel/" in url:
            ...
            await status_msg.edit_text(
                "⚠️ *Unsupported Instagram link*\n\nPlease send a reel or post link.",
                parse_mode="Markdown"
            )
            return
        ...
        post = await loop.run_in_executor(
            None, lambda: instaloader.Post.from_shortcode(_IL.context, shortcode)
        )

        video_url = post.video_url if post.is_video else post.url
        caption   = (post.caption or "Instagram Reel")[:80]
        username  = post.owner_username
        dur_str   = ""
        if post.is_video and post.video_duration:
            m, s    = divmod(int(post.video_duration), 60)
            dur_str = f"⏱️  {m}m {s}s\n"

        keyboard = InlineKeyboardMarkup([[
            InlineKeyboardButton("▶️  Watch / Save Video", url=video_url)
        ]])
...
# ═════════════════════════════════════════════
#  YT-DLP CORE  (all other platforms)
# ═════════════════════════════════════════════
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✨ *Welcome, {name}!*\n\n"
        "Send me any video link and I'll fetch it for you — fast & clean.\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "🔗 *Just send a link!*\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "📦 *Supported Platforms*\n"
        ...
        "💡 *Tip:* You can choose 🎬 Video *or* 🎵 Audio!\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "5️⃣  Instagram: tap the button to open & save\n"
        "5️⃣  Others: choose 🎬 *Video* or 🎵 *Audio*\n"
        "6️⃣  Done! ✅\n\n"
        "⚠️ Content must be *public* to download.",
        parse_mode="Markdown"
    )
...
async def check_join(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await is_member(update, context):
        await query.message.edit_text(
            f"🎉 *Welcome, {name}!*\n\n"
            "You're all set! Send me any video link. 🚀",
            parse_mode="Markdown"
        )
    else:
        await query.message.edit_text(
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Instagram: direct button method ──
    if "instagram.com" in url:
        await handle_instagram(url, status_msg, uid)
        return
    ...
    try:
        ...
        if info.get("view_count"):
            ...
            view_str = (
                f"👁️  {vc:,} views\n"
                if vc < 1_000_000
                else f"👁️  {vc/1_000_000:.1f}M views\n"
            )
